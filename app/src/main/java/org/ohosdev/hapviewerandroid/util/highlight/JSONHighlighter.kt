...
object JSONHighlighter {
    ...
    fun highlight(
        ...
        highlightedJson =
            PATTERN_STRING.replace(highlightedJson) { "<span style=\"color: $stringColor;\">${it.groupValues[1]}</span>" }
        highlightedJson =
            PATTERN_KEYWORD.replace(highlightedJson) { "<span style=\"color: $keywordColor;\">${it.value}</span>" }
        highlightedJson =
            PATTERN_NUMBER.replace(highlightedJson) { "<span style=\"color: $numberColor;\">${it.groupValues[1]}</span>" }
